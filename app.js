...
db.connect((err) => {
    ...
    CREATE TABLE IF NOT EXISTS arquivos (
      ...
      hmac LONGBLOB NOT NULL,
      arquivo LONGBLOB NOT NULL,
      arquivo_hash_assinado LONGBLOB NOT NULL
    );`;
...
app.post('/upload', upload.fields([{ name: "ficheiro", maxCount: 1 }, { name: "ficheiro2", maxCount: 1 }]), (req, res) => {
    ...
    db.query(selectQuery, (err, result) => {
        ...
        const file = req.files["ficheiro"][0];
        const file2 = req.files["ficheiro2"][0];

        // Verificar se o arquivo existe
        if (!file || !file2) {
            return res.status(301).redirect('/?msg=Um ou vários arquivos não enviados');
        }

        var bufferFicheiro = file.buffer;

        //Importa chave privada do arquivo PEM
        const privateKey2File = file2.buffer.toString();

        //Constrói o objeto da chave privada
        const privateKey2 = forge.pki.privateKeyFromPem(privateKey2File);

        //Assinatura utilizando hash e com chave privada
        let messageDigest = forge.md.sha256.create();
        messageDigest.update(forge.util.encodeUtf8(bufferFicheiro), 'utf8');
        let arquivo_hash_assinado = privateKey2.sign(messageDigest);


        let fileHMAC = ""
        let chave_hmac = ""
        let hash_hmac = ""
        if (req.body.hash_hmac && req.body.chave_hmac) {
            let hmac = crypto.createHmac(req.body.hash_hmac, req.body.chave_hmac);
            hmac.update(bufferFicheiro);
            fileHMAC = hmac.digest('hex');
            chave_hmac = req.body.chave_hmac;
            hash_hmac = req.body.hash_hmac
        }
        ...
        if (req.body.comprimir) {
            if (req.body.comprimir == 'sim') {
                ...
                comprimir(file.buffer, file).then((bufferFicheiro) => {
                    ...
                    messageDigest.update(forge.util.encodeUtf8(bufferFicheiro), 'utf8');
                    arquivo_hash_assinado = privateKey2.sign(messageDigest);
                    ...
                    if (req.body.cifrado == 'nao') {
                        salvarDataBase(file.originalname + '.zip', 0, sendTo, encrypted, chaveAesCifrada, true, fileHMAC, bufferFicheiro, arquivo_hash_assinado) //este 'bufferFicheiro' == return da funcao 'comprimir()'
                    }
                    ...
                    if (req.body.cifrado == 'sim') {
                        ...
                        salvarDataBase(file.originalname + '.zip', ivBase64, sendTo, encrypted, encryptedAes, true, fileHMAC, bufferFicheiroBase64, arquivo_hash_assinado) //este 'bufferFicheiro' == return da funcao 'comprimir()'
                ...
                if (req.body.cifrado == 'sim') {
                    ...
                    cifrarFicheiro_e_AES_salvarDB(userData, file, arquivo_hash_assinado, chave_hmac, hash_hmac)
                    ...
                    const sendTo = req.body.token_dest;
                    salvarDataBase(file.originalname, false, sendTo, encrypted, chaveAesCifrada, false, fileHMAC, bufferFicheiro, arquivo_hash_assinado)
                }
            }
        } else {
    ...
    function salvarDataBase(nome, iv, sendTo, encrypted, encrypted_aes, zip, fileHMAC, arquivo, arquivo_hash_assinado) {
        const insertQuery = 'INSERT INTO arquivos (nome, iv, sendTo, encrypted, encrypted_aes, zip, hmac, arquivo, arquivo_hash_assinado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
        db.query(insertQuery, [nome, iv, sendTo, encrypted, encrypted_aes, zip, fileHMAC, arquivo, arquivo_hash_assinado], (err, result) => {
            if (err) {
                throw err;
            }
    ...
    function cifrarFicheiro_e_AES_salvarDB(userData, file, arquivo_hash_assinado, chaveHmac, hash_hmac) {
        let encrypted = true;
        var bufferFicheiro = file.buffer;
        ...
        let fileHMAC = "";
        if (chaveHmac != "" && hash_hmac != "") {
            let hmac = crypto.createHmac(hash_hmac, chaveHmac);
            hmac.update(bufferFicheiro);
            fileHMAC = hmac.digest('hex');
        }

        salvarDataBase(file.originalname, ivBase64, sendTo, encrypted, encryptedAes, false, fileHMAC, bufferFicheiroBase64, arquivo_hash_assinado)
    }
...
app.post('/download', upload.fields([{ name: "privateKeyFile", maxCount: 1 }, { name: "publicKeyFile", maxCount: 1 }]), (req, res) => {
    const fileId = req.body.id;

    // Consultar o banco de dados para obter o arquivo pelo ID
    const selectQuery = "SELECT nome, iv, arquivo, arquivo_hash_assinado, encrypted, encrypted_aes, zip FROM arquivos WHERE id = '" + fileId + "';";
    db.query(selectQuery, [fileId], (err, result) => {
        ...
        const signedHashOfFile = fileData.arquivo_hash_assinado;
        ...
        const file = req.files["privateKeyFile"][0];
        const file2 = req.files["publicKeyFile"][0];

        //Importar chave pública do suposto emissor, para validação da assinatura
        const publicKey2FromFile = file2.buffer.toString();

        //Construir chave pública
        const publicKey2 = forge.pki.publicKeyFromPem(publicKey2FromFile);
        // Enviar o conteúdo do arquivo DECIFRADO como resposta
        if (encrypted == '1') {
            if (!file || !file2) {
                return res.status(301).redirect('/?msg=Um ou vários ficheiros não selecionador!');
            }

            // try {
            // Importar chave privada do formulário
            const pemPrivateKey = file.buffer.toString();
            ...
            //Verificação utilizando a chave privada e a assinatura da base de dados
            let messageDigest = forge.md.sha256.create();
            messageDigest.update(forge.util.encodeUtf8(dadosDecifrados), 'utf8');
            let arquivo_hash_verificado = publicKey2.verify(messageDigest.digest().bytes(), signedHashOfFile.toString().slice(0, 256));

            if (arquivo_hash_verificado) {
                res.send(dadosDecifrados);
                //return res.status(301).redirect('/?msg= O arquivo foi enviado pelo emissor esperado!');
            }
            else {
                return res.status(301).redirect('/?msg= O arquivo não foi enviado pelo emissor esperado!');
            }
            ...
            //Verificação utilizando a chave privada e a assinatura da base de dados
            let messageDigest = forge.md.sha256.create();
            messageDigest.update(fileData.arquivo, 'utf8');
            let arquivo_hash_verificado = publicKey2.verify(messageDigest.digest().bytes(), signedHashOfFile.toString().slice(0, 256));

            if (arquivo_hash_verificado)
                res.send(fileData.arquivo);
            else
                return res.status(301).redirect('/?msg= O arquivo não foi enviado pelo emissor esperado!');
        }
    });
